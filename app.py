...
introduction_text = open(os.path.join(
    text_path, "introduction.md"), "r").read()
                    ...
                    [
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                ...
                                {'label': 'Average Chemical Potential',
                                    'value': 'chempot'},
                        ...
                        html.Div(id='chem-slider-container', children=[
                            dcc.Slider(
                                ...
                                min=0,
                                marks={int(
                                    i*0.1) if not i % 10 else i*0.1: "{:.1f}".format(i*0.1) for i in range(0, 46, 5)},
                                step=0.01,
                                value=1,
                                updatemode='drag'
                            ),
                        ...
                        html.Div(id='temp-slider-container', children=[
                            dcc.Slider(
                                ...
                                step=0.01,
                                marks={
                                    i*0.1: "{:.1f}".format(i*0.1) + 'K' for i in range(11)},
                                updatemode='drag',
                                value=0.5
                            ),
                        ], style={'display': 'none'}),
                    ...
                    [
                        ...
                        html.Div(id='expectation-value-container', children=[
                            dcc.Checklist(
                                ...
                                options=[
                                    {'label': 'Average Electron Position',
                                        'value': 'ticked'}
                                ]
                            )
                        ], style={'display': 'none'}),
...
def pass_system_data(fig_dict):
    return dict(
        min=dict(zip(range(1, 3), [
                 min(fig_dict['data'][0]['y']), min(fig_dict['data'][1]['y'])])),
        max=dict(zip(range(1, 3), [
                 max(fig_dict['data'][0]['y']), max(fig_dict['data'][1]['y'])])),
    ),
